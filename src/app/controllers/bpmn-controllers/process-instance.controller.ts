...
@ApiBearerAuth('JWT')
@ApiTags('ProcessInstance')
@Controller('/api/v1/ProcessInstance')
export class ProcessInstanceController {
  constructor(
    protected readonly processService: ProcessInstanceService,
  ) { }
  ...
  @ApiBadRequestResponse({ description: 'Have error in run time' })
  public async findById(@Param('id') id: string): Promise<ProcessInstanceVM> {
  ...
  @ApiBadRequestResponse({ description: 'Have error in run time' })
  public async remove(@Param('id') id: string): Promise<ProcessInstanceVM> {
